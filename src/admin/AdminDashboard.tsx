...
import { listenProducts, listenOrders, createProduct, updateProduct, deleteProduct, listenCategories, addCategoryIfNotExists, ensureCategories, reindexAllProductsSearchFields } from "../services/firestoreService";
...
import { BarChart3, Home, Package, ShoppingCart, Users as UsersIcon, LayoutDashboard, ShieldCheck } from "lucide-react";
...
const AdminDashboard: React.FC = () => {
  ...
  const defaultCategories = ["Men", "Women", "Shoes", "Bags", "Accessory"];
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      await createProduct({
        ...
        promotionPercent: Math.max(0, Math.min(100, Number(newProduct.promotionPercent) || 0)),
        ...
        colors: parseColors(newColorsRaw),
      });
      setToast({ message: "Product added successfully.", type: "success" });
      // Stay on Add Product modal for rapid entry; reset form for next product
      setShowAddModal(true);
      setNewProduct({ name: "", price: 0, promotionPercent: 0, category: "", image: "", description: "", stock: 0, rating: 0, isNewArrival: false, colors: [] });
      setNewColorsRaw('');
      setImageFile(null);
  ...
  const handleReindexSearch = async () => {
    const ok = confirm('Reindex search for all products? This will update existing products so search like "shirt" works better.');
    if (!ok) return;
    try {
      setSaving(true);
      setToast(null);
      const count = await reindexAllProductsSearchFields();
      setToast({ message: `Search index updated for ${count} products.`, type: 'success' });
    } catch (e: any) {
      console.error(e);
      setToast({ message: 'Failed to reindex products.', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
                ...
                <div className="card metric">
                  ...
                  <div>
                    ...
                    <div className="metric-value">
                      ${orders.filter(o => o.status !== OrderStatus.CANCELLED).reduce((sum, o) => sum + (o.total || 0), 0).toFixed(2)}
                    </div>
                  </div>
                </div>
                <div className="card metric">
                    ...
                    const monthTotals = months.map((_, i) =>
                      orders
                        ...
                        .filter(o => {
                          ...
                          return Number.isFinite(d.getTime()) && d.getMonth() === i;
                        })
                        .reduce((sum, o) => sum + (o.total || 0), 0)
                    );
                    const maxMonthly = Math.max(1, ...monthTotals);
                    return months.map((m, i) => {
                      const monthTotal = monthTotals[i] || 0;
                      const h = Math.min(180, (monthTotal / maxMonthly) * 180);
                      return (
          ...
          <section className="products-section">
            <div className="section-header">
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                <button className="btn" disabled={saving} onClick={handleReindexSearch}>Reindex Search</button>
              </div>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <input
                  ...
                  type="number"
                  placeholder="0 - 100"
                  value={Number(newProduct.promotionPercent || 0)}
                  onChange={(e) => setNewProduct({ ...newProduct, promotionPercent: Math.max(0, Math.min(100, Number(e.target.value || 0))) } as any)}
                />
            ...
            <div className="modal-footer">
              ...
              >{saving ? (typeof uploadProgress === 'number' ? `Saving… ${uploadProgress}%` : 'Saving…') : "Add Product"}</button>
        ...
        <div className="modal-overlay" onClick={() => setShowEditModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <input
                  ...
                  type="number"
                  placeholder="0 - 100"
                  value={Number((editTarget as any).promotionPercent || 0)}
                  onChange={(e) => setEditTarget({ ...editTarget, promotionPercent: Math.max(0, Math.min(100, Number(e.target.value || 0))) } as any)}
                />
                <label className="form-label">Stock</label>
                <input
                  className="input"
                ...
                <input
                  ...
                  type="text"
                  placeholder="e.g. #000000, #a0522d"
                  value={editColorsRaw || (Array.isArray((editTarget as any).colors) ? ((editTarget as any).colors as string[]).join(', ') : '')}
                  onChange={(e) => { setEditColorsRaw(e.target.value); setEditTarget({ ...editTarget, colors: parseColors(e.target.value) } as any); }}
                />
            ...
            <div className="modal-footer">
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    await handleUpdateProduct(editTarget.id, {
                      ...
                      promotionPercent: Math.max(0, Math.min(100, Number((editTarget as any).promotionPercent) || 0)),
                      stock: editTarget.stock,
                      category: editTarget.category,
                      image: imageUrl,
                      description: editTarget.description,
                      isNewArrival: !!editTarget.isNewArrival,
                      colors: parseColors(editColorsRaw || ''),
                    });
              ...
              >{saving ? (typeof editUploadProgress === 'number' ? `Saving… ${editUploadProgress}%` : 'Saving…') : "Update"}</button>
            </div>
          </div>
        </div>
      )}
